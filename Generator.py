...
class Gen():
    def __init__(self, itm_cnt, usr_cnt, dim_hidden, n_time_step, learning_rate, grad_clip, emb_dim, lamda=0.2, initdelta=0.05,MF_paras=None,model_type="rnn",use_sparse_tensor=True,update_rule='adam'):
        ...
        self.weight_initializer = tf.compat.v1.random_uniform_initializer(minval=-0.05, maxval=0.05)
        self.const_initializer = tf.compat.v1.constant_initializer(0.0)
        self.emb_initializer = tf.compat.v1.random_uniform_initializer(minval=-0.05, maxval=0.05)


        self.sparse_tensor=use_sparse_tensor

        self.item_sequence = tf.compat.v1.placeholder(tf.float32, [None, self.T, self.V_U])
        #print(self.item_sequence)
...
#----------#
        ...
        with tf.compat.v1.variable_scope('MF'):
            if self.MF_paras is None:
                self.user_embeddings = tf.Variable(
                    tf.random.uniform([self.V_U, self.emb_dim], minval=-self.initdelta, maxval=self.initdelta,
                                      dtype=tf.float32))
                self.item_embeddings = tf.Variable(
                    tf.random.uniform([self.V_M, self.emb_dim], minval=-self.initdelta, maxval=self.initdelta,
                                      dtype=tf.float32))
    ...
    def _decode_lstm(self, h_usr, h_itm, reuse=False):
        if False:
            #print("phase7")
            with tf.compat.v1.variable_scope('G_rating', reuse=reuse):
                w_usr = tf.compat.v1.get_variable('w_usr', [self.H, self.H], initializer=self.weight_initializer)
                b_usr = tf.compat.v1.get_variable('b_usr', [self.H], initializer=self.const_initializer)
                w_itm = tf.compat.v1.get_variable('w_itm', [self.H, self.H], initializer=self.weight_initializer)
                b_itm = tf.compat.v1.get_variable('b_itm', [self.H], initializer=self.const_initializer)

                usr_vec = tf.matmul(h_usr, w_usr) + b_usr

                itm_vec = tf.matmul(h_itm, w_itm) + b_itm

                out_preds = tf.reduce_sum(input_tensor=tf.multiply(usr_vec, itm_vec), axis=1)
                self.paras_rnn.extend([w_usr,b_usr,w_itm,b_itm])
                return out_preds
        else:
            #print("phase9")
            out_preds = tf.reduce_sum(input_tensor=tf.multiply(h_usr, h_itm), axis=1)
...
#----------#
    def _get_initial_lstm(self, batch_size):
        with tf.compat.v1.variable_scope('G_initial_lstm'):
...
#----------#
    def _item_embedding(self, inputs, reuse=False):
        with tf.compat.v1.variable_scope('G_item_embedding', reuse=reuse):
            w = tf.compat.v1.get_variable('w', [self.V_U, self.H], initializer=self.emb_initializer)
...
#----------#
    def _user_embedding(self, inputs, reuse=False):
        with tf.compat.v1.variable_scope('G_user_embedding', reuse=reuse):
           w = tf.compat.v1.get_variable('w', [self.V_M, self.H], initializer=self.emb_initializer)
...
#-------------#
        batch_size = tf.shape(input=self.item_sequence)[0]
        print(tf.shape(input=self.item_sequence))
        #print("\nphase1 sidhfshg\n")
        #print(batch_size)
...
#---------#
        itm_lstm_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=self.H)
        usr_lstm_cell = tf.compat.v1.nn.rnn_cell.LSTMCell(num_units=self.H)
...
#----------#
        ...
        self.loss_RNN = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=self.rating, logits=self.logits_RNN)) #+ RNN_Regularizer
#----------#

        self.logits_MF = tf.reduce_sum(input_tensor=tf.multiply(self.u_embedding, self.i_embedding), axis=1) + self.i_bias  +self.u_bias
...
#----------#
        ...
        self.joint_loss = tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(labels=self.rating, logits=self.pre_joint_logits))
#----------#
        ...
        self.joint_loss += self.lamda * tf.reduce_sum(input_tensor=[tf.nn.l2_loss(para) for para in self.paras_rnn])
...
#----------#
        ...
        if self.model_type == 'joint':
            grads = tf.gradients(ys=self.joint_loss, xs=tf.compat.v1.trainable_variables())
        elif self.model_type == 'rnn':
            grads = tf.gradients(ys=self.loss_RNN, xs=tf.compat.v1.trainable_variables())
        elif self.model_type == 'mf':
            grads = tf.gradients(ys=self.loss_MF, xs=tf.compat.v1.trainable_variables())
#----------#
...
#----????---#
        ...
        self.all_logits = tf.reduce_sum(input_tensor=tf.multiply(self.u_embedding, self.item_embeddings), axis=1) + self.item_bias + self.u_bias
#----????---#

        self.reward = tf.compat.v1.placeholder(tf.float32)
        if self.model_type == 'rnn':
            self.pg_loss = - tf.reduce_mean(input_tensor=tf.math.log( tf.sigmoid(self.logits_RNN)) * self.reward)
            self.pg_loss += self.lamda * tf.reduce_sum(input_tensor=[tf.nn.l2_loss(para) for para in self.paras_rnn])
        else:
            self.pg_loss = - tf.reduce_mean(input_tensor=tf.math.log( tf.sigmoid(self.pre_joint_logits)) * self.reward)
        ...
        pg_grads = tf.gradients(ys=self.pg_loss, xs=tf.compat.v1.trainable_variables())
        pg_grads_and_vars = list(zip(pg_grads, tf.compat.v1.trainable_variables()))
